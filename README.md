# 常用的 Web 字体包

## 免责声明

该项目仅供学习记录，不作任何商业用途！

该仓库中所包含的字体文件仅用于个人学习目的。任何商业用途，包括但不限于出售、分发或用于营利性项目均属违规。使用者应自行承担因违反此声明而引起的任何法律责任。

## 版权声明

- 所有字体文件的版权归原始制作者所有。
- 该仓库仅提供字体的收集和整理，不对字体文件的版权拥有任何所有权。

请在使用字体文件之前务必详细阅读并理解以上声明。任何使用本仓库中字体文件的行为将被视为已经阅读、理解并同意遵守以上声明。


## 字体包


### `Acens`

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Facens)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Facens)

```shell
npm install @zf-web-font/acens --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `Acens` | 现代风格的无衬线字体，线条简洁且具有未来感，适用于科技和时尚品牌的标志设计。 | |


### `Digital`

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fdigital)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fdigital)

```shell
npm install @zf-web-font/digital --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `Digital` | 电子显示风格的字体，模仿LCD或LED显示屏上的数字和字符的外观，常用于显示板和科技主题的界面。 | |


### `DIN`

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fdin)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fdin)

```shell
npm install @zf-web-font/din --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `DIN Medium` | 德国工业标准字体，特点是清晰和功能性，常用于交通标志、技术文档和工程设计中。 | |


### `Montserrat Ace`

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fmontserrat-ace)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fmontserrat-ace)

```shell
npm install @zf-web-font/montserrat-ace --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `Montserrat Ace Bold Italic` | 加粗斜体的现代无衬线字体，具有圆润的字形和动感的斜体效果，适合强调重点和动态内容。 | |
| `Montserrat Ace Bold` | 加粗的现代无衬线字体，线条坚实而清晰，适用于标题和重要文本。 | |
| `Montserrat Ace Italic` | 斜体的现代无衬线字体，字形优雅且流畅，常用于突出轻松或重要的信息。 | |
| `Montserrat Ace Light Italic` | 轻便斜体的现代无衬线字体，字体轻盈且具有艺术感，适合创意和设计导向的内容。 | |
| `Montserrat Ace Light` | 轻便的现代无衬线字体，字体细腻且适合大段文字的阅读。 | |
| `Montserrat Ace Medium Italic` | 中等粗细斜体的现代无衬线字体，平衡了风格与可读性，适用于多种文本环境。 | |
| `Montserrat Ace Medium` | 中等粗细的现代无衬线字体，通用性强，适合多种文本和显示需求。 | |
| `Montserrat Ace Regular` | 标准粗细的现代无衬线字体，风格中性且易于阅读，适用于正文和通用文字。 | |
| `Montserrat Ace SemiBold Italic` | 半加粗斜体的现代无衬线字体，具有显著的风格特征，适用于需要强调和吸引注意的文本。 | |
| `Montserrat Ace SemiBold` | 半加粗的现代无衬线字体，提供更强的视觉冲击，适合突出显示的文本或标题。 | |


### `OPPOSans`

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fopposans)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fopposans)

```shell
npm install @zf-web-font/opposans --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `OPPOSans B` / `OPPOSans Bold` | 加粗的无衬线字体，设计简洁，适用于品牌和营销材料中的强调文字。 | |
| `OPPOSans H` / `OPPOSans Heavy` | 极粗的无衬线字体，强调力度和存在感，常用于广告和海报中的显著标题。 | |
| `OPPOSans L` / `OPPOSans Light` | 轻便的无衬线字体，线条细腻优雅，适合大量文本阅读和高级设计。 | |
| `OPPOSans M` / `OPPOSans Medium` | 中等粗细的无衬线字体，平衡了风格和实用性，适用于各种文字和背景。 | |
| `OPPOSans R` / `OPPOSans Regular` | 标准粗细的无衬线字体，通用且易于阅读，适合日常书写和正文。 | |


### `PangMenZhengDaoBiaoTi` *（旁门正道标题）*

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fpangmenzhengdaobiaoti)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fpangmenzhengdaobiaoti)

```shell
npm install @zf-web-font/pangmenzhengdaobiaoti --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `PangMenZhengDaoBiaoTi` / `PangMenZhengDaoBiaoTi Regular` | 现代感强烈的中文无衬线标题字体，特点是字形大胆且具有视觉冲击力，适合广告和标志 | |


### `SourceHanSansCN` *（思源黑体）*

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fsourcehansanscn)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fsourcehansanscn)

```shell
npm install @zf-web-font/sourcehansanscn --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `SourceHanSansCN Bold` | 加粗的无衬线字体，特点是坚实和显眼，适用于需要强调和突出显示的文本。 | |
| `SourceHanSansCN ExtraLight` | 极细的无衬线字体，线条纤细透明，适合在大背景和空间感设计中使用，提供一种轻盈感。 | |
| `SourceHanSansCN Heavy` | 极粗的无衬线字体，提供最强的视觉冲击力，适用于需要非常显眼的标题或重要信息标注。 | |
| `SourceHanSansCN Light` | 轻便的无衬线字体，具有优雅和精致的感觉，适合阅读体验和细节展示。 | |
| `SourceHanSansCN Medium` | 中等粗细的无衬线字体，平衡了风格和可读性，适合商务和专业文档。 | |
| `SourceHanSansCN Normal` | 标准粗细的无衬线字体，提供中性的文字风格和良好的阅读体验，适合日常用途。 | |
| `SourceHanSansCN Regular` | 标准粗细的无衬线字体，线条清晰，适合文本和段落中的常规应用。 | |


### `YouSheBiaoTiHei` *（优设标题黑）*

![NPM Downloads](https://img.shields.io/npm/dw/%40zf-web-font%2Fyoushebiaotihei)
![jsDelivr hits (npm)](https://img.shields.io/jsdelivr/npm/hw/%40zf-web-font%2Fyoushebiaotihei)

```shell
npm install @zf-web-font/youshebiaotihei --save
```

| 字体 | 描述 | 预览 |
| -- | -- | -- |
| `YouSheBiaoTiHei` / `YouSheBiaoTiHei Regular` | 这是一款现代黑体系列的无衬线字体，特点是字形饱满且对比度适中，适用于正式或商务的文档和展示，提供清晰的视觉效果和良好的阅读体验。 |


## 使用方法

安装对应的字体的 npm 包，在项目中引用样式 `style.css` 或者 `style.scss` 文件。也可以使用带有 cdn 缓存的版本 `style-cdn.css` 或者 `style-cdn.scss`

以 `Acens` 字体为例

NPM 安装字体：

```shell
npm install @zf-web-font/acens --save
```

引用样式文件：

* 无 CDN 版本：
  * JS 文件引用： `import "@zf-web-font/acens/style.css"`
  * CSS 文件引用： `@import "@zf-web-font/acens/style.css"`
  * SCSS 文件引用： `@import "@zf-web-font/acens/style.scss"`
* CDN版本
  * JS 文件引用： `import "@zf-web-font/acens/style-cdn.css"`
  * CSS 文件引用： `@import "@zf-web-font/acens/style-cdn.css"`
  * SCSS 文件引用： `@import "@zf-web-font/acens/style-cdn.scss"`

通过 css 参数 `font-family` 使用 字体

```css
.text {
  font-family: "Acens";
}
